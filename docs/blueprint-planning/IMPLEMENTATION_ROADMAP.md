...
| Seq | Module | Ticket slices | Blocking markers | Required test plan |
|---:|---|---|---|---|
| 3 | `lesson-details-attendance` | Calendar event detail panel; mobile teacher attendance marking; unmarked worklist; student lesson history; D-05 event adapter at module boundary; explicit teacher/admin lesson-row preparation per accepted D-17. | D-17 is accepted: one `lesson_records` row per event/student, no embedded event-level attendance container, no silent event-open materialization, and prepared rows start unconfirmed. | `RLS-LIVE` teacher own insert/update vs other denied; `PW-SMOKE` event -> mark attendance -> history/counter; `RTL-MOBILE` attendance 390x844; `MAP-UNIT` attendance helpers; `BACKFILL` explicit teacher/admin lesson-record preparation. |
| 4 | ✅ `payroll-salaries-hours` | Teacher self-report path; admin approval/pay flow; variance worklist; payslip rows; finance read/export path. | D-18 implemented: `HoursEntry` source of truth, `HoursReport` period header. D-19 implemented: configurable rate policy, payable rate stamped at admin approval. | `RLS-LIVE` teacher own DRAFT/SUBMITTED only, finance read/export only, admin approve/pay; `PW-SMOKE` submit -> compare -> approve -> payslip; `RTL-MOBILE` teacher submit; `MAP-UNIT` variance/rate tests; `BACKFILL` period headers/archive context passed. |
| 5 | ✅ `payments-charges` | Route top-level Finance using `BILLING`; family-led ledger list/detail; charge/payment/adjustment posting; live balance on demand and audit snapshots. | D-20 implemented: single-currency P0 with future-safe multi-currency mode; instrument deposit rows remain `BLOCKED ON D-25`. | `RLS-LIVE` admin/finance only, plain member denied, cross-org denied; `PW-SMOKE` charge -> payment -> balance -> void; `RTL-MOBILE` readable finance states; `MAP-UNIT` partial allocation, single-currency invariant tests; `BACKFILL` familyId ledger linking. |
...
| 15 | `operations-command-center` | Admin Inbox operations summary; pure source-authorized snapshot cards; open conflicts, today events, inbox/import/report-health cards; finance-only card subset. | Source-specific cards still `BLOCKED ON D-21` through `BLOCKED ON D-27`; attendance/payroll/finance cards should use accepted D-17/D-18/D-19/D-20 models after source modules ship. Guardian/contact cards may use D-16 jsonb path only after student/family source authorization exists. | `RLS-LIVE` admin full, finance allowed cards only, no hidden-count leakage, no anon; `PW-SMOKE` Admin Inbox summary -> card drill-downs; `RTL-MOBILE` permission/blocked states; `MAP-UNIT` exported command helpers; no dashboard backfill. |
...
3. Keep P0 attendance, payroll, and finance next. D-17 through D-20 are answered
   for the P0 path; remaining blocked sections start at D-21 and D-25 for
   adjacent operational/instrument finance scope.
