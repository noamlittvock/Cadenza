...
| 5 | `payments-charges` | Route top-level Finance using `BILLING`; family-led ledger list/detail; charge/payment/adjustment posting; live balance on demand and audit snapshots. | D-20 accepted: single-currency P0 with future-safe multi-currency mode; instrument deposit rows `BLOCKED ON D-25`. | `RLS-LIVE` admin/finance only, plain member denied, cross-org denied; `PW-SMOKE` charge -> payment -> balance -> void; `RTL-MOBILE` readable finance states; `MAP-UNIT` partial allocation, single-currency invariant tests; `BACKFILL` familyId ledger linking. |
