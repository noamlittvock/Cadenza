...
test.describe('DevTools — Role Simulator', () => {
  ...
  test('#54 activating a role card shows simulation banner', async ({ page }) => {
    ...
    await page.getByText('Admin (Active)').first().click();

    // Banner appears with the simulated role label
    await expect(page.getByText('Simulating: Admin (Active)')).toBeVisible({ timeout: 5_000 });
  });
  ...
  test('#55 re-clicking active card deactivates simulation', async ({ page }) => {
    ...
    await page.getByText('SuperAdmin').first().click();
    await expect(page.getByText('Simulating: SuperAdmin')).toBeVisible({ timeout: 5_000 });
  ...
  test('#58 Viewer role hides edit/write actions', async ({ page }) => {
    ...
    await page.getByText('Viewer (Read-Only)').first().click();
    await expect(page.getByText('Simulating: Viewer (Read-Only)')).toBeVisible({ timeout: 5_000 });
  ...
  test('Clear role button deactivates role simulation', async ({ page }) => {
    ...
    await expect(page.getByText('Simulating: Admin (Active)')).not.toBeVisible({ timeout: 3_000 });
  });
});
