...
export const FinancialAnalysis: React.FC<Props> = ({ events, teachers, settings, savedCharts, setSavedCharts, onMobileMenuOpen, onNavigateBack }) => {
    ...
    const insightTiles: InsightTile[] = useMemo(() => {
        ...
        return [
            ...
            { id: 'avg-cost', title: 'Avg Cost / Teacher', icon: <Target size={16} />, value: formatCurrency(avgCost), context: `Across ${rwd.length} active teacher${rwd.length !== 1 ? 's' : ''}`, highlight: 'neutral', gradient: 'bg-gradient-to-r from-violet-400 to-purple-600' },
            ...
            { id: 'cost-split', title: 'Hourly vs Global', icon: <CreditCard size={16} />, value: `${hourlyPct.toFixed(0)}% / ${(100 - hourlyPct).toFixed(0)}%`, context: `${formatCurrency(totals.hourlyCost)} hourly · ${formatCurrency(totals.globalCost)} global`, highlight: 'neutral', gradient: 'bg-gradient-to-r from-blue-400 to-emerald-500' },
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-8">
                        {savedCharts.map(chart => (
                            <div key={chart.id} className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden group">
                                ...
                                {/* Body */}
                                <div className="p-4"><ChartRenderer config={chart} events={filteredEvents} teachers={teachers} height={280} /></div>
                                {/* Footer */}
